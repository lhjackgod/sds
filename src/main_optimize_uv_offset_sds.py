...
def _load_init_scale(init_dir: Path, mesh: MeshData, fallback_scale: np.ndarray) -> np.ndarray:
    for name in ("optimized_offset_scale.npy", "offset_scale.npy"):
        path = init_dir / name
        if path.exists():
            data = np.load(path).astype(np.float32)
            if len(data) == len(mesh.vertices):
                return data
    return fallback_scale.astype(np.float32)
...
def run(args: argparse.Namespace) -> None:
    ...
    init_dir = Path(args.init_offset_dir)
    ...
    init_scale = _load_init_scale(init_dir, mesh, rule_result.vertex_init)
    ...
    log = {**result.log, "offset_stats": _stats(result.offset_scale)}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--init-offset-dir", required=True)
    ...
    parser.add_argument("--guidance-scale", type=float, default=20.0)
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--sds-mode", choices=("dummy", "castex"), default="dummy")
    ...
    parser.add_argument("--lambda-lap", type=float, default=0.1)
